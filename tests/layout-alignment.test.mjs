...
assert.match(script, /textContent = xmlMode \? '解析为 JSON' : 'Json 模式'/);
...
assert.match(script, /if \(codeMode\) setCodeMode\(false\);[\s\S]{0,60}else if \(xmlMode\) convertXmlToJson\(\);[\s\S]{0,60}else void formatJson\(\);/);
