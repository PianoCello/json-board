...
assert.doesNotMatch(html, /data-action="line-numbers"/);
assert.match(css, /\.line-numbers\s*\{[\s\S]*?display:\s*block/);
...
assert.match(html, /data-action="xml-mode"/);
assert.doesNotMatch(html, /data-action="xml-to-json"/);
assert.match(script, /const canParseXml = xmlMode && xmlConvertible/);
assert.match(script, /textContent = canParseXml \? '解析为 JSON' : 'Json 模式'/);
...
assert.match(html, /src="\.\/src\/js\/xml-utils\.js\?v=/);
...
assert.match(css, /\.fold-button\s*\{[\s\S]*?width:\s*15px;[\s\S]*?height:\s*15px/);
assert.match(css, /\.fold-button::after\s*\{[^}]*inset:\s*-4px/);
...
assert.match(script, /function enableLineClipboard\(/);
assert.match(script, /function restoreEditorHistory\(/);
assert.match(script, /key === 'z'/);
assert.match(script, /event\.ctrlKey && \(key === 'y' \|\| event\.code === 'KeyY'\)/);
assert.match(script, /event\.inputType !== 'historyUndo'/);
assert.match(script, /clearTimeout\(renderTimer\);[\s\S]{0,120}clearTimeout\(diagnosticTimer\)/);
assert.match(script, /addEventListener\('copy'/);
assert.match(script, /addEventListener\('cut'/);
assert.match(script, /function collapseAllFolds\(/);
assert.match(script, /JsonBoardEditor\.lineNumbersForRange\(start, end, numberMap\)/);
assert.doesNotMatch(css, /body\.code-mode \.fold-controls\s*\{\s*display:\s*none/);
assert.doesNotMatch(script, /if \(compareMode \|\| codeMode\) \{ foldControls\.replaceChildren\(\); return; \}/);
